...
def convert_qwen3_weights(qwen, cfg: HookedTransformerConfig):
    """Convert Qwen3 model weights to TransformerLens format.
    
    Qwen3 differs from Qwen2 in that:
    - It doesn't have biases in the attention layers
    - It uses Q/K normalization (RMSNorm applied after Q and K projection)
    """
    ...
    assert cfg.n_key_value_heads is not None  # Qwen3 uses GQA
    ...
    for l in range(cfg.n_layers):
        ...
        # Qwen3 doesn't have attention biases, so we set them to zero
        ...
        # Qwen3 Q/K normalization weights
        state_dict[f"blocks.{l}.attn.q_norm_w"] = qwen.model.layers[l].self_attn.q_norm.weight
        state_dict[f"blocks.{l}.attn.k_norm_w"] = qwen.model.layers[l].self_attn.k_norm.weight
